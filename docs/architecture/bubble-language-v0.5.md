...
This release is the first organic self-realization kernel, not the completed bubble universe. `v0.5.1` later proves an explicit agent-bearing connected seam, `v0.5.2` adds actor-neutral multi-frontier causal closure with generic emergence, and `v0.5.3` adds bounded causal-configuration recurrence separated from monotone anchor history, boundary-mediated conjunctive law hypercuts, identity, universal memory evidence, recurrent-reference restoration, and outward influence. General coinductive/open-ended dynamics, goals and derived agency, concrete branch/spawn/collapse execution, global sea dynamics, and a full identity/time calculus remain live work. External observer-agent and comparison work remains Phase 2.
