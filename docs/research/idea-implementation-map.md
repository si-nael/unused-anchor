...
| A world may change without universal time | Q-018, Q-019 | No-universal-clock self-realization, reciprocal reversibility, causal order, branching/terminal topology, commit-only history arrow | Partial | Add explicit cycles or causal partial order from a concrete world requirement |
...
| Q-002 negative sea | Partial | Source aggregation exists; dynamics do not | Feed concrete realization consequences back into sea dynamics |
| Q-003 positive sea | Partial | Structural support exists; placement/growth dynamics do not | Feed lineage continuations back into placement/growth dynamics |
...
| Q-019 time inside a bubble | Partial | Order classes exist without causal graph, cycles, duration, or synchronization | Extend only from a concrete world law |
...
Before `v0.5.1`, the repository must:
...
- pass mandatory `verify:self-realization` and full repository verification
- choose one connected Phase 1 continuation seam from the `v0.5` release queue
