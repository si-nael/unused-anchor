...
async function main(): Promise<void> {
    ...
    if (!inputPath) {
        throw new Error("Usage: tsx apps/hatchery/materialize-bubble.ts <input.bubble> [output.json] [--observation-policy-override <commit-single-observed-region|commit-hidden-region-with-latent-bubble-siblings|defer-multiple-hidden-region-targets|defer-no-eligible-observed-target>]");
    }
    ...
    const materialized = materializeBubbleProgram(result.program, options);
...
function parseCliArgs(argv: string[]): {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (argument === "--observation-policy-override") {
            ...
            observationCommitPolicyOverride = createObservationCommitPolicyOverride(rawRule, "materialize CLI override");
            index += 1;
            continue;
        }
