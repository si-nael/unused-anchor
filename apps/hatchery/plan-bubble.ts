...
import { planBubbleProgram } from "../../src/bubbles/runtime";

async function main(): Promise<void> {
    const { inputPath, outputPath } = parseCliArgs(process.argv.slice(2));

    if (!inputPath) {
        throw new Error("Usage: tsx apps/hatchery/plan-bubble.ts <input.bubble> [output.json]");
    }
    ...
    const plan = planBubbleProgram(result.program);
    ...
    process.stdout.write(`Planned ${inputPath} -> ${outputPath}\n`);
}
